...
SSThreshold("lvm-ssection-threshold", cl::Hidden,
            cl::desc("Small data and bss section threshold size (default=8)"),
            cl::init(8));

void LVMTargetObjectFile::Initialize(MCContext &Ctx, const TargetMachine &TM){
  ...
  this->TM = &static_cast<const LVMTargetMachine &>(TM);
}
...
IsGlobalInSmallSectionImpl(const GlobalObject *GO,
  ...
  const LVMSubtarget &Subtarget =
      *static_cast<const LVMTargetMachine &>(TM).getSubtargetImpl();
