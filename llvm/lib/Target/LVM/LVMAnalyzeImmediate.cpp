...
LVMAnalyzeImmediate::Inst::Inst(unsigned O, unsigned I) : Opc(O), ImmOperand(I) {}

// Add I to the instruction sequences.
void LVMAnalyzeImmediate::AddInstr(InstSeqLs &SeqLs, const Inst &I) {
...
void LVMAnalyzeImmediate::GetInstSeqLsADDiu(uint64_t Imm, unsigned RemSize, InstSeqLs &SeqLs) {
  GetInstSeqLs((Imm + 0x8000ULL) & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(LDR, Imm & 0xffffULL));
}

void LVMAnalyzeImmediate::GetInstSeqLsORi(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
  GetInstSeqLs(Imm & 0xffffffffffff0000ULL, RemSize, SeqLs);
  AddInstr(SeqLs, Inst(ORi, Imm & 0xffffULL));
}

void LVMAnalyzeImmediate::GetInstSeqLsSHL(uint64_t Imm, unsigned RemSize,
                                           InstSeqLs &SeqLs) {
...
void LVMAnalyzeImmediate::GetInstSeqLs(uint64_t Imm, unsigned RemSize,
                                        InstSeqLs &SeqLs) {
  ...
  if (RemSize <= 16) {
    AddInstr(SeqLs, Inst(LDR, MaskedImm));
    return;
  }
...
// are replaced with
//  LUi 0x444
void LVMAnalyzeImmediate::ReplaceADDiuSHLWithLUi(InstSeq &Seq) {
  // Check if the first two instructions are ADDiu and SHL and the shift amount
  // is at least 16.
  if ((Seq.size() < 2) || (Seq[0].Opc != LDR) ||
...
void LVMAnalyzeImmediate::GetShortestSeq(InstSeqLs &SeqLs, InstSeq &Insts) {
...
const LVMAnalyzeImmediate::InstSeq &LVMAnalyzeImmediate::Analyze(uint64_t Imm, unsigned Size,
